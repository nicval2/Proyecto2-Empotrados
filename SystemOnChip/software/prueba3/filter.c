 ...
 *
 * Shelf filters usando combinación de LP/HP con ganancia
 */
...
static int16_t lowpass(int16_t x, int32_t *y_state, int alpha_shift) {
    /* alpha = 1 / (2^alpha_shift), típicamente 2-4 */
...
int16_t filter_process(int16_t x) {
    ...
    switch (current_filter) {
        ...
        case FILTER_LOWPASS:
            /* Filtro técnico: solo graves */
            yn = lowpass(x, &y_prev, 2);
            break;

        case FILTER_HIGHPASS:
            /* Filtro técnico: solo agudos */
...
int filter_update_from_switches(int sw_value) {
    ...
    /* Decodificar switches - prioridad al más alto */
