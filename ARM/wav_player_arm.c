// wav_player_arm.c - HPS con handshake de inicialización (v2.5)
...
#define CMD_READY       0x52454459  /* "REDY" - Señal del NIOS */
...
/* --- GLOBALES --- */
static volatile uint8_t *fifo_in_csr_base = NULL;
static volatile uint32_t *fifo_in = NULL;
static volatile uint8_t *fifo2_out_csr_base = NULL;
static volatile uint32_t *fifo2_out = NULL;
static void *h2f_map = NULL;
static int fd_mem = -1;
...
void fifo_write_string(const char *str) {
    ...
    if (len > 63) len = 63;
    fifo_write_wait(len);
    for (size_t i = 0; i < len && keep_running; i++) {
...
void check_commands(void) {
    ...
    while (fifo2_has_data() && count < 10) {
        ...
        /* Ignorar CMD_READY durante reproducción */
    }
}
...
int play_file(const char *filename) {
    ...
    while (keep_running && 
           (samples_read = fread(audio_buffer, sizeof(int16_t),
        ...
        if (num_channels == 2) {
            for (size_t i = 0; i < samples_read && keep_running; i += 2) {
                int32_t mono = ((int32_t)audio_buffer[i] + audio_buffer[i+1]) / 2;
                fifo_write_wait((uint32_t)(mono & 0xFFFF));
            }
        } else {
            for (size_t i = 0; i < samples_read && keep_running; i++) {
...
/* --- ESPERAR SEÑAL READY DEL NIOS --- */
int wait_for_nios_ready(void) {
    int wait_time = 0;
    
    printf("Esperando señal READY del NIOS...\n");
    printf("(Carga el .sof desde Quartus)\n\n");
    
    while (keep_running) {
        /* Verificar si hay datos en FIFO2 */
        if (fifo2_has_data()) {
            uint32_t cmd = *fifo2_out;
            
            if (cmd == CMD_READY) {
                printf("Señal READY recibida del NIOS!\n");
                return 1;
            }
            /* Descartar otros comandos durante la espera */
        }
...
int main(int argc, char *argv[]) {
    ...
    printf("   Con Handshake de Inicializacion\n");
    printf("================================================\n");
    printf("Pistas: %d\n", total_tracks);
